...
import TokenomicsSections from "./animated";
...
function TokenomicsSection() {
  ...
  useEffect(() => {
    intervalRefs.current = languages.map(
      (_, index) =>
        setInterval(() => {
          setLanguages((prev) =>
            prev.map((lang, i) =>
  ...
  return (
    <section
      ref={ref}
      id="token"
      className="py-20 px-4 relative max-w-7xl mx-auto"
    >
      <h1 className="font-bold text-[46px] leading-[55px] my-10">Tokenomics</h1>
      <motion.div
        className="grid md:grid-cols-2 gap-6 px-5"
        initial={{ opacity: 0, y: 20 }}
        animate={inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
        transition={{ duration: 0.6, delay: 0.2 }}
      >
        {/* Image Section */}
        <div className="w-full flex items-center justify-center h-full min-h-[750px] relative max-w-[550px] max-h-[600px]">
          <Image
            src="/chart.svg"
        ...
        {/* Information Cards */}
        {["liquidityPool", "teamAllocation", "treasury"].map(
          (section, index) => (
            <Card
              key={index}
              section={section}
...
function Card({ section, language, onMouseEnter, onMouseLeave }) {
  const { transform, opacity } = useSpring({
    opacity: 1,
    transform: `perspective(600px) rotateY(${language === "en" ? 0 : 180}deg)`,
    config: { mass: 5, tension: 500, friction: 80 },
  });

  return (
    <animated.div
      className="p-4 bg-white/15 rounded-[8px] h-full min-h-[750px] backdrop-blur-lg relative"
    ...
    >
      ...
      >
        <CardContent section={section} language="zh" />
      </div>
      <animated.div
        style={{
          opacity,
      ...
      >
        <CardContent section={section} language="en" />
      </animated.div>
    </animated.div>
  );
}

function CardContent({ section, language }) {
  return (
    <IntlProvider messages={messages[language]} locale={language}>
      <div className="flex flex-col gap-4 p-4">
        {/* Title */}
        <div className="flex gap-2 text-[18px] lg:text-[30px]">
          <div className="font-medium flex items-center gap-2">
            <div
              className="w-4 h-5 lg:w-5 lg:h-7 rounded-full"
              style={{
                backgroundColor:
                  section === "liquidityPool"
                    ? "#3983FF"
                    : section === "teamAllocation"
                    ? "white"
                    : "#EA8010",
              }}
            />
        ...
        {/* Description */}
        <div className="text-[14px] lg:text-[22px] lg:px-8 space-y-3">
          <p>
            <FormattedMessage id={`${section}Desc1`} />
          </p>
          <p>
            <span className="font-semibold mr-1">
              <FormattedMessage id={`${section}Desc2Title2`} />
            </span>
            <FormattedMessage id={`${section}Desc2`} />
          </p>
          <p>
            <span className="font-semibold mr-1">
              <FormattedMessage id={`${section}Desc3Title3`} />
            </span>
            <FormattedMessage id={`${section}Desc3`} />
          </p>
          {section === "teamAllocation" || section === "treasury" ? (
            <p>
              <span className="font-semibold mr-1">
                <FormattedMessage id={`${section}Desc4Title4`} />
              </span>
              <FormattedMessage id={`${section}Desc4`} />
            </p>
          ) : null}
        </div>
      </div>
